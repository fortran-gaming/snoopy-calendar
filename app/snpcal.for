...
C      OPEN (UNIT=7,FILE='SNPCAL.OUT',ACTION='WRITE',STATUS='REPLACE')
      ...
      character(4) :: argv
      integer :: ierr, L
      if (command_argument_count() < 1) stop 'Specify year e.g. 2023'
      call get_command_argument(1, argv, length=L, status=ierr)
      if (ierr /= 0 .or. L /= 4) stop 'Specify 4-digit year e.g. 2023'
      read(argv, '(I4)') iyr
      iyrlst = iyr + 1
      end block
